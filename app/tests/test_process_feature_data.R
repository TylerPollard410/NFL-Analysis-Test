...
srs_last <- srs_raw |> add_week_seq() |> slice_tail(n = 1, by = team)
srs_feat <- srs_raw |> 
  #add_week_seq()
  dplyr::arrange(season, week) |> 
  # no internal lag; use add_lag after if desired
  dplyr::select(season, week, team, dplyr::all_of(srs_cols))

srs_feat <- dplyr::left_join(base_df, srs_feat, by = c("season", "week", "team"))
#}
