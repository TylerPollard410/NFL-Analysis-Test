...
system.time(
  for (fold_name in names(rolling_folds)) {
    ...
    fold_rmse <- rmse(test_df$total, fold_pred)
    fold_mae  <- mae(test_df$total, fold_pred)
    ...
    temp_pred_df <- test_df %>%
      select(game_id, season, week, total) %>%
...
global_rolling_cv <- function(max_depth, 
  ...
  for (f_name in names(folds_list)) {
    ...
    rmse_val <- rmse(test_df$total, pred_test)
...
evaluate_global_best <- function(hparams, 
  ...
  for (f_name in names(folds_list)) {
    ...
    fold_rmse <- rmse(test_df$total, test_pred)
    fold_mae  <- mae(test_df$total, test_pred)
...
rolling_forecast <- function(data, 
  ...
  for (w in unique_weeks) {
    ...
    train_set <- rbind(train_set, current_test %>% select(-pred_total))
  }
...
overall_rmse <- rmse(all_forecasts$total, all_forecasts$pred_total)
overall_mae <- mae(all_forecasts$total, all_forecasts$pred_total)
...
yearly_summary <- all_forecasts %>%
  group_by(season) %>%
  summarise(
    RMSE = rmse(total, pred_total),
    MAE  = mae(total, pred_total),
    n    = n()
  )
...
home_team_summary <- all_forecasts %>%
  group_by(home_team) %>%
  summarise(
    RMSE = rmse(total, pred_total),
    MAE  = mae(total, pred_total),
...
print(home_team_summary, n = nrow(home_team_summary))
...
ggplot(home_team_summary, aes(x = home_team, y = RMSE)) +
  geom_line() +
  geom_point() +
...
ggplot(all_forecasts, aes(x = total, y = pred_total)) +
  geom_point(alpha = 0.6, color = "blue") +
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +
  smplot2::sm_statCorr() +
  labs(title = "Actual vs Predicted total",
       x = "Actual total",
       y = "Predicted total") +
  theme_minimal()
...
ggplot(all_forecasts, aes(x = total, y = pred_total)) +
  ...
  smplot2::sm_statCorr() +
  facet_wrap(~ season) +
  labs(title = "Actual vs Predicted total by Forecast Year",
       x = "Actual total",
       y = "Predicted total") +
  theme_minimal()
...
ggplot(all_forecasts, aes(x = total, y = pred_total)) +
  ...
  smplot2::sm_statCorr() +
  facet_wrap(~ week) +
  labs(title = "Actual vs Predicted total Score by Forecast Year",
       x = "Actual total Score",
       y = "Predicted total Score") +
  theme_minimal()


# 9. Save Model Info -----
final_xgb_model_total <- final_model_full
final_xgb_params_total <- final_params
final_xgb_importance_total <- importance_matrix_gain
final_xgb_forecasts_total<- all_forecasts

file_loc <- "~/Desktop/NFL Analysis Data/finalXGBmodels/"

save(final_xgb_model_total,
     file = paste0(file_loc, "final_xgb_model_total", ".rda"))
save(final_xgb_params_total,
     file = paste0(file_loc, "final_xgb_params_total", ".rda"))
save(final_xgb_importance_total,
     file = paste0(file_loc, "final_xgb_importance_total", ".rda"))
save(final_xgb_forecasts_total,
     file = paste0(file_loc, "final_xgb_forecasts_total", ".rda"))
...
load(file = paste0(file_loc, "final_xgb_forecasts_total", ".rda"))
...
xgb_cv_preds <- nfl_data |> 
  ...
  left_join(
    final_xgb_forecasts_total |>
      select(game_id, xgb_total = pred_total),
    by = join_by(game_id)
  ) |>
