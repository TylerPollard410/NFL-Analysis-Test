...
ggplot(data = modelDataPlot) +
  geom_histogram(
    ...
    color = "lightblue3", fill = "lightblue", bins = 70) +
  geom_density(
    aes(x = result),
    color = "lightblue4", 
    linewidth = 1) +
...
#### Continuous ----
ggplot(data = modelDataPlot, aes(x = home_SRS, y = result)) +
...
ggplot(data = modelDataPlot, aes(x = surface, y = result)) +
  geom_point(alpha = 0.1) +
  geom_density2d() +
  theme_bw()
...
Fit <- brm(
  ...
  data = modelDataTrain,
  family = brmsfamily(family = "poisson"),
...
spreadFitResultLogical <- gameTrainResult > gameTrainSpread
spreadFitResultProb <- mean(spreadFitBetLogical == spreadFitResultLogical)
spreadFitResultProb
...
spreadDataTrain <- modelDataTrainNA |>
  ...
  mutate(
    spreadFit = spreadFitMean,
    spreadBet = ifelse(spreadFit > spread_line, "Home", "Away"),
    spreadResult = ifelse(result > spread_line, "Home", "Away"),
    spreadSuccess = spreadBet == spreadResult,
    ...
    spreadCoverSuccess = spreadCoverBet == spreadResult
  )
...
spreadPredsResultLogical <- gameTestResult > gameTestSpread
spreadPredsResultProb <- mean(spreadPredsBetLogical == spreadPredsResultLogical)
...
spreadDataTest <- modelDataTestNA |>
  select(game_id, season, week, game_type,
         ...
         away_spread_odds, away_spread_prob) |>
  mutate(
    spreadPreds = spreadPredsMean,
    spreadBet = ifelse(spreadPreds > spread_line, "Home", "Away"),
    spreadResult = ifelse(result > spread_line, "Home", "Away"),
    spreadSuccess = spreadBet == spreadResult,
