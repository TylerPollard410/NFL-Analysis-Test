...
nfl_data_long <- modDataLong |> filter(season >= 2007)
nfl_data <- modData |> filter(season >= 2007)
...
nfl_data_long <- nfl_data_long |> mutate(time_id = as.numeric(paste0(season, sprintf("%02d", week))))
...
drop_vars_long <- c("game_id", "season", "game_type", "week", "team", "opponent",
               "team_score", "opponent_score", "result", "spread_line", "spreadCover",
               "total", "total_line", "totalCover", "winner", "team_spread_odds", "team_spread_prob",
               "opponent_spread_odds", "opponent_spread_prob", "over_odds", "over_prob",
               "under_odds", "under_prob", "team_moneyline", "team_moneyline_prob",
               "opponent_moneyline", "opponent_moneyline_prob", "stadium", "team_coach", "opponent_coach",
               "time_id", "overtime", "FEATURE_COLS")

drop_vars <- c("game_id", "season", "game_type", "week", 
               "home_team", "away_team",
               "home_score", "away_score", "result", "spread_line", "spreadCover",
               "total", "total_line", "totalCover", "winner", "home_spread_odds", "home_spread_prob",
               "away_spread_odds", "away_spread_prob", "over_odds", "over_prob",
               "under_odds", "under_prob", "home_moneyline", "home_moneyline_prob",
               "away_moneyline", "away_moneyline_prob", "stadium", "home_coach", "away_coach",
               "time_id")

# Define game-level variables (which we will exclude from XGBoost and later reintroduce in Bayesian modeling)
game_level_vars_long <- c(
  "season_type", "weekday", "time_of_day", "location", "locationID", "div_game", 
  "team_rest", "opponent_rest",
  "roof", "surface", "temp", "wind"
)

game_level_vars <- c(
  "season_type", "weekday", "time_of_day", "location", "location2", "div_game", 
  "home_rest", "away_rest",
  "roof", "surface", "temp", "wind"
)

# Candidate predictors for XGBoost: all numeric variables not in drop_vars and not in game_level_vars.
all_candidates <- setdiff(names(nfl_data_long), drop_vars_long)
candidate_xgb_vars <- all_candidates[ sapply((nfl_data_long)[, all_candidates], is.numeric) ]
candidate_xgb_vars <- setdiff(candidate_xgb_vars, game_level_vars_long)
...
## 2.A. Preprocessing: ----
candidate_data <- nfl_data_long |> select(all_of(candidate_xgb_vars))

# Keep only complete.cases
complete_rows <- complete.cases(candidate_data)
complete_rows_ID <- nfl_data_long |> filter(complete_rows) |> pull(game_id)
missing_rows_ID <- nfl_data_long |> filter(!complete_rows) |> pull(game_id)
candidate_data <- candidate_data |> filter(complete_rows)

missing_data <- nfl_data_long |> 
  filter(game_id %in% missing_rows_ID) |> 
  select(game_id, all_of(candidate_xgb_vars))

NAcols <- apply(missing_data, 2, function(x) sum(is.na(x)))
NAcols[NAcols != 0]
...
system.time(
  for (fold_name in names(rolling_folds)) {
    ...
    temp_pred_df <- test_df %>%
      ...
      mutate(pred_result = fold_pred, fold_name = fold_name)
...
global_rolling_cv <- function(max_depth, 
  ...
  for (f_name in names(folds_list)) {
    ...
    rmse_val <- rmse(test_df[[target_var]], pred_test)
...
rolling_forecast <- function(data, 
  ...
  for (w in unique_weeks) {
    ...
    current_test$pred_result <- preds
    forecast_preds <- rbind(forecast_preds, current_test)
    
    # Update train_set by adding the actual outcomes for week w.
    # (Here we assume the actual outcomes are in current_test$result.)
    train_set <- rbind(train_set, current_test %>% select(-pred_result))
  }
...
overall_rmse <- rmse(all_forecasts[[target_var]], all_forecasts$pred_result)
overall_mae <- mae(all_forecasts[[target_var]], all_forecasts$pred_result)
...
yearly_summary <- all_forecasts %>%
  group_by(season) %>%
  summarise(
    RMSE = rmse(!!sym(target_var), pred_result),
    MAE  = mae(!!sym(target_var), pred_result),
    n    = n()
  )
...
weekly_summary <- all_forecasts %>%
  group_by(week) %>%
  summarise(
    RMSE = rmse(get(target_var), pred_result),
    MAE  = mae(get(target_var), pred_result),
    n    = n()
  )
print(weekly_summary, n = nrow(weekly_summary))

# Summarize performance by forecast week
team_summary <- all_forecasts %>%
  group_by(team) %>%
  summarise(
    RMSE = rmse(!!sym(target_var), pred_result),
    MAE  = mae(!!sym(target_var), pred_result),
...
print(team_summary, n = nrow(team_summary))

# Summarize performance by forecast week
away_team_summary <- all_forecasts %>%
  group_by(away_team) %>%
  summarise(
    RMSE = rmse(get(target_var), pred_result),
    MAE  = mae(get(target_var), pred_result),
...
print(away_team_summary, n = nrow(away_team_summary))
...
ggplot(home_team_summary, aes(x = team, y = RMSE)) +
  geom_line() +
  geom_point() +
  labs(title = "Forecast RMSE by Week", x = "Forecast Home Team", y = "RMSE") +
  theme_minimal()

ggplot(away_team_summary, aes(x = away_team, y = RMSE)) +
  geom_line() +
  geom_point() +
  labs(title = "Forecast RMSE by Week", x = "Forecast Away Team", y = "RMSE") +
  theme_minimal()


# Basic scatter plot of Actual vs. Predicted home scores
ggplot(all_forecasts, aes(x = result, y = pred_result)) +
...
# 9. Save Model Info -----
final_xgb_model_result <- final_model_full
final_xgb_params_result <- final_params
final_xgb_importance_result <- importance_matrix_gain
final_xgb_forecasts_result <- all_forecasts

file_loc <- "~/Desktop/NFL Analysis Data/finalXGBmodels/"

save(final_xgb_model_result,
     file = paste0(file_loc, "final_xgb_model_result", ".rda"))
save(final_xgb_params_result,
     file = paste0(file_loc, "final_xgb_params_result", ".rda"))
save(final_xgb_importance_result,
     file = paste0(file_loc, "final_xgb_importance_result", ".rda"))
save(final_xgb_forecasts_result,
     file = paste0(file_loc, "final_xgb_forecasts_result", ".rda"))
...
load(file = paste0(file_loc, "final_xgb_forecasts_result", ".rda"))
...
save(xgb_cv_preds,
     file = paste0(file_loc, "xgb_cv_preds_native", ".rda"))
