...
generated quantities {
  ...
  for (k in 1:N_oos) {
    ...
    // One-step state for target week
    vector[N_teams] ts_next;
    if (w == 1 && s > 1) {
      // find last global week of previous season
      int w_prev = 1;
      for (g in 1:N_games) if (season_idx[g] == s - 1 && week_idx[g] > w_prev) w_prev = week_idx[g];
      vector[N_teams - 1] z_raw;
      for (t in 1:(N_teams - 1)) z_raw[t] = normal_rng(0, 1);
      vector[N_teams] z0 = sum_to_zero_vector_constrain(z_raw);
      ts_next = phi_season_carryover * team_strength[w_prev] + z0 * sigma_season_carryover;
    } else if (w > 1 && w - 1 >= 1 && (w - 1) <= N_weeks) {
      vector[N_teams - 1] z_raw;
      for (t in 1:(N_teams - 1)) z_raw[t] = normal_rng(0, 1);
      vector[N_teams] z0 = sum_to_zero_vector_constrain(z_raw);
      ts_next = phi_weekly * team_strength[w - 1] + z0 * sigma_weekly_innovation;
    } else {
