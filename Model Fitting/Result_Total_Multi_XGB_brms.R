...
formula_result_total <- formula_result + formula_total

default_prior(formula_result_total, histModelData)

priors_result_total <- c(
...
system.time(
  fit_result_total <- brm(
    formula_result_total,
    data = histModelData,
    #prior = priors_Team,
...
#plot(fit_Team)
print(fit_result_total, digits = 4)

#fit_Team <- fit_Team2
pp_check(fit_result_total, resp = "result", ndraws = 100, type = "bars")
pp_check(fit_result_total, resp = "total", ndraws = 100, type = "bars")
pp_check(fit_result_total, resp = "result", ndraws = 100, type = "dens_overlay")
pp_check(fit_result_total, resp = "total", ndraws = 100, type = "dens_overlay")

pp_check(fit_result_total, newdata = modelData, 
...
save(fit_Team, file = "~/Desktop/NFL Analysis Data/fit_result_total.RData")

### Fixed Effects ----
fixedEff_result_total <- fixef(fit_result_total)
fixedEff_result_total <- data.frame(fixedEff_result_total) |>
  mutate(
...
print(fixedEff_result_total, digits = 4)
fixedSigEff_result_total <- fixedEff_result_total |> filter(p_val < 0.2)
print(fixedSigEff_result_total)

randEff_result_total <- ranef(fit_result_total)
randEff_result_total
...
fitRT <- 3
assign(paste0("fit_Team", fitRT), fit_result_total)
assign(paste0("fixedEff_result_total", fitRT), fixedEff_result_total)
assign(paste0("randEff_result_total", fitRT), randEff_result_total)
...
## Comparison Table ----
accuracy_metrics_RT <- tibble(
  Fit = rep(paste0("Fit", fit), 4),
  Data = rep(c("Train", "Test"), each = 2),
  Bet = rep(c("result", "total"), times = 2),
