...
## Plot ----
### Histogram ----
modelDataPlot <- modelDataLong |> 
  filter(season >= 2021) |>
  filter(!(season == 2024 & week > 9))

ggplot(data = modelDataPlot) +
  geom_histogram(
    aes(x = team_score, after_stat(density)),
...
ggplot(data = modelDataPlot, aes(color = location, group = location)) +
  geom_histogram(
    aes(x = team_score, after_stat(density)),
    #color = "lightblue3", fill = "lightblue", 
    bins = 70) +
...
#### Continuous ----
ggplot(data = modelDataPlot, 
       aes(x = team_SRS_mov, y = team_score)) +
...
ggplot(data = modelDataPlot, 
       aes(x = team_SRS_mov, y = team_score, color = week)) +
  geom_point(alpha = 0.1) +
  geom_smooth() +
  #sm_statCorr() +
  #facet_wrap(vars(week)) +
...
#### Categorical
ggplot(data = modelData, aes(x = home_team, y = result)) +
...
modelDataTrain <- modelDataLong |>
  filter(season %in% seasonsTrain) |>
  mutate(
    across(where(is.numeric) & !c(team_score, opponent_score,
                                  result, spread_line, 
                                  cover,
                                  team_spread_odds, opponent_spread_odds,
                                  team_spread_prob, opponent_spread_prob,
                                  total, total_line, 
                                  under_odds, over_odds,
                                  under_prob, over_prob,
                                  team_moneyline, opponent_moneyline,
                                  team_moneyline_prob, opponent_moneyline_prob),
           function(x){scale(x)})
  )
str(modelDataTrain)
...
Fit <- brm(
  bf(team_score ~ 
       ...
       #team_off_pass_epa_adj + opponent_def_pass_epa_adj +
       #team_off_rush_epa_adj + opponent_def_rush_epa_adj +
       #team_def_pass_epa_adj + opponent_off_pass_epa_adj +
       #team_def_rush_epa_adj + opponent_off_rush_epa_adj +
       #game_type +
       #div_game +
       #roof +
       #surface +
...
teamScoreTrain <- modelDataTrainNA$team_score
teamScoreTest <- modelDataTestNA$team_score
predMetrics <- tibble(
  Fit = paste0("Fit", fit),
  MAE_fit = mean(abs(FittedMean - teamScoreTrain)),
  MAD_fit = mean(abs(FittedMed - teamScoreTrain)),
  COV_fit = mean(FittedLCB < teamScoreTrain & teamScoreTrain < FittedUCB),
  MAE_pred = mean(abs(PredsMean - teamScoreTest), na.rm = TRUE),
  MAD_pred = mean(abs(PredsMed - teamScoreTest), na.rm = TRUE),
  COV_pred = mean(PredsLCB < teamScoreTest & teamScoreTest < PredsUCB)
)
predMetrics

#### Prob Errors ----
##### Fit ----
spreadTrain <- modelDataTrainNA$spread_line
#spreadTrain <- as.numeric(spreadTrainScale*attr(spreadTrainScale, "scaled:scale") + attr(spreadTrainScale, "scaled:center"))

FittedProbs <- matrix(NA, nrow = sims, ncol = length(resultTrain))
for(j in 1:length(resultTrain)){
  fitted <- Fitted[, j]
  probs <- fitted > spreadTrain[j]
  FittedProbs[, j] <- probs
}
FittedBet <- colMeans(FittedProbs)
FittedBetLogical <- FittedBet > 0.5
FittedResultLogical <- resultTrain > spreadTrain
FittedResultProb <- mean(FittedBetLogical == FittedResultLogical, na.rm = TRUE)
FittedResultProb

resultDataTrain <- modelDataTrainNA |>
...
resultSuccessTrain <- resultDataTrain |>
  summarise(
    CoverProbTrain = mean(coverSuccess, na.rm = TRUE),
    spreadCoverProbTrain = mean(spreadCoverSuccess, na.rm = TRUE)
  )
resultSuccessTrain

##### Pred ----
spreadTest <- modelDataTestNA$spread_line
#spreadTest <- as.numeric(spreadTestScale*attr(spreadTrainScale, "scaled:scale") + attr(spreadTrainScale, "scaled:center"))

PredsProbs <- matrix(NA, nrow = sims, ncol = length(resultTest))
for(j in 1:length(resultTest)){
  fitted <- Preds[, j]
  probs <- fitted > spreadTest[j]
  PredsProbs[, j] <- probs
}
PredsBet <- colMeans(PredsProbs)
PredsBetLogical <- PredsBet > 0.5
PredsResultLogical <- resultTest > spreadTest
PredsResultProb <- mean(PredsBetLogical == PredsResultLogical, na.rm = TRUE)
PredsResultProb

resultDataTest <- modelDataTestNA |>
  select(game_id, season, week, game_type,
         team, team_score, opponent, opponent_score,
         result, spread_line, cover,
         team_spread_odds, team_spread_prob,
...
resultSuccessTest <- resultDataTest |>
  summarise(
    CoverProbTest = mean(coverSuccess, na.rm = TRUE),
    spreadCoverProbTest = mean(spreadCoverSuccess, na.rm = TRUE)
  )
resultSuccessTest

##### Combined Success ----
resultSuccess <- bind_cols(
  Fit = paste0("Fit", fit),
  resultSuccessTrain,
  resultSuccessTest
)
...
resultSuccessComb <- bind_rows(
  resultSuccessComb,
  resultSuccess
) |>
  arrange(desc(spreadCoverProbTest))
resultSuccessComb
...
save(predMetricsComb, resultSuccessComb, looComb, fitFormulas,
     file = "./Data/resultLongFitDiagnostics.RData")
...
#### Fit ----
##### Home ----
ppc_dens_overlay(y = modelDataTestNA$result, yrep = Preds) +
