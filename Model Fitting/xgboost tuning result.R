...
system.time(
  for (fold_name in names(rolling_folds)) {
    ...
    dtest_fold <- xgb.DMatrix(
      ...
      label = test_df$result
    )
    
    fold_pred <- predict(xgb_final_model, dtest_fold)
    fold_rmse <- rmse(test_df$result, fold_pred)
    fold_mae  <- mae(test_df$result, fold_pred)
    ...
    temp_pred_df <- test_df %>%
      select(game_id, season, week, result) %>%
...
global_rolling_cv <- function(max_depth, 
  ...
  for (f_name in names(folds_list)) {
    ...
    rmse_val <- rmse(test_df$result, pred_test)
...
evaluate_global_best <- function(hparams, 
  ...
  for (f_name in names(folds_list)) {
    ...
    fold_rmse <- rmse(test_df$result, test_pred)
    fold_mae  <- mae(test_df$result, test_pred)
...
overall_rmse <- rmse(all_forecasts$result, all_forecasts$pred_result)
overall_mae <- mae(all_forecasts$result, all_forecasts$pred_result)
...
yearly_summary <- all_forecasts %>%
  group_by(season) %>%
  summarise(
    RMSE = rmse(result, pred_result),
    MAE  = mae(result, pred_result),
    n    = n()
  )
