...
transformed parameters {
  ...
  matrix[N_teams, N_weeks] team_strength_raw = rep_matrix(0, N_teams, N_weeks);
  matrix[N_teams, N_weeks] team_strength; // centered by week (plain mean)
...
model {
  ...
  // --- Likelihood over the first N_obs games (init + accumulate) ---
  {
    ...
    for (g in 1:N_obs) {
      ...
      mu_obs[g] = (team_strength[i, w] - team_strength[j, w]) + team_hfa[i, s] * hfa[g];
