...
parameters {
  array[N_weeks] sum_to_zero_vector[N_teams] strength;   // [week][team], sum-to-zero per week
  matrix[N_teams, N_seasons] hfa;                        // Team-specific HFA per season
...
transformed parameters {
  vector[N_games] mu;
  for (i in 1:N_games) {
    mu[i] = strength[game_week[i], home_id[i]]
          - strength[game_week[i], away_id[i]]
          + hfa[home_id[i], game_season[i]];
  }
}
model {
  ...
  for (s in 2:N_seasons) {
    ...
    strength[w_start] ~ normal(rho_season * strength[w_prev_end], tau_season);
  }
  ...
  for (s in 1:N_seasons) {
    ...
    for (w in (w_start + 1):w_end) {
      strength[w] ~ normal(rho_week * strength[w-1], tau_week);
    }
  }
