// ssm1.stan
// HFA + team-strength state-space (zero-sum strengths; team HFA deviations zero-sum per season)
...
generated quantities {
  ...
  // Optional: out-of-sample predictions by index into N_games (no likelihood contribution)
  // vector[N_oos] mu_oos;
  // vector[N_oos] y_pred;
  // 
  // for (k in 1:N_oos) {
  ...
  //   int w = week_id[g];
  //   int s = season_id[g];
  //   mu_oos[k] = (team_strength[w][i] - team_strength[w][j]) + team_hfa[s][i] * hfa[g];
  //   y_pred[k] = normal_rng(mu_oos[k], sigma_y);
  // }

  // (Optional compact snapshots if you want:)
  // array[N_seasons] vector[N_teams] last_strength;
  // for (s in 1:N_seasons) last_strength[s] = team_strength[last_week_of_season[s]];
}
