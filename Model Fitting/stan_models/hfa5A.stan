data {
  ...
  array[N_weeks]   int<lower=1, upper=N_seasons> week_season;         // not used, ok to keep
...
parameters {
  ...
  real<lower=0, upper=1> beta_w;        // within-season AR(1) coef
  real<lower=0>          sigma_w;       // within-season shock scale

  real<lower=0, upper=1> beta_s;        // between-season carryover
  real<lower=0>          sigma_s;       // between-season start shock
...
transformed parameters {
  ...
  // Build EACH SEASON completely before moving on
  for (s in 1:N_seasons) {
