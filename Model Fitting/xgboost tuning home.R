...
system.time(
  for (fold_name in names(rolling_folds)) {
    ...
    dtest_fold <- xgb.DMatrix(
      ...
      label = test_df$home_score
    )
    
    fold_pred <- predict(xgb_final_model, dtest_fold)
    fold_rmse <- rmse(test_df$home_score, fold_pred)
    fold_mae  <- mae(test_df$home_score, fold_pred)
    ...
    temp_pred_df <- test_df %>%
      select(game_id, season, week, home_score) %>%
      mutate(pred_home_score = fold_pred, fold_name = fold_name)
...
global_rolling_cv <- function(max_depth, 
  ...
  for (f_name in names(folds_list)) {
    ...
    rmse_val <- rmse(test_df$home_score, pred_test)
...
evaluate_global_best <- function(hparams, 
  ...
  for (f_name in names(folds_list)) {
    ...
    fold_rmse <- rmse(test_df$home_score, test_pred)
    fold_mae  <- mae(test_df$home_score, test_pred)
...
dtrain_full <- xgb.DMatrix(
  ...
  label = nfl_data_model$home_score
)
...
rolling_forecast <- function(data, 
  ...
  for (w in unique_weeks) {
    ...
    dtrain <- xgb.DMatrix(
      ...
      label = current_train$home_score
    )
    dtest <- xgb.DMatrix(
    ...
    current_test$pred_home_score <- preds
    forecast_preds <- rbind(forecast_preds, current_test)
    
    # Update train_set by adding the actual outcomes for week w.
    # (Here we assume the actual outcomes are in current_test$home_score.)
    train_set <- rbind(train_set, current_test %>% select(-pred_home_score))
  }
...
overall_rmse <- rmse(all_forecasts$home_score, all_forecasts$pred_home_score)
overall_mae <- mae(all_forecasts$home_score, all_forecasts$pred_home_score)
...
yearly_summary <- all_forecasts %>%
  group_by(season) %>%
  summarise(
    RMSE = rmse(home_score, pred_home_score),
    MAE  = mae(home_score, pred_home_score),
    n    = n()
  )
...
ggplot(all_forecasts, aes(x = home_score, y = pred_home_score)) +
